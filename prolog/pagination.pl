...
%! http_link_header(+Result) is semidet.

http_link_header(Result) :-
  pagination_iris(Result, Pairs),
  format("Link: "),
  http_link_header0(Pairs),
  nl.

http_link_header0([]) :- !.
http_link_header0([H]) :- !,
  http_link_header_comp0(H).
http_link_header0([H|T]) :- !,
  http_link_header_comp0(H),
  format(", "),
  http_link_header0(T).

http_link_header_comp0(Rel-Iri) :-
