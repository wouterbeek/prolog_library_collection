:- module(
  marcxml,
  [
    marcxml_record/2, % +Source, -Record
    marcxml_test/0
  ]
).
...
marcxml_test :-
  pp_dict(marcxml_record(file('iish.evergreen.authorities.xml'))).
...
marc_subfield('035', '', '', a, Val, Key-Val) :- !,
...
%! marcxml_dom_controlfield(+Dom, -Pair) is det.

marcxml_dom_controlfield(element('marc:controlfield',[tag=Key1],[Val1]), Key-Val2) :-
  maplist(strip_atom, [Key1,Val1], [Key2,Val2]),
  rdf_global_id(marc:Key2, Key).



%! marcxml_dom_datafield(+Dom, +Field, +Subfield, -Pair) is nondet.
%! marcxml_dom_datafield(+Dom, +Field, -Indicator1, -Indicator2, +Subfield, -Pair) is nondet.
...
marcxml_dom_datafield(
  ...
  marcxml_dom_subfield(Tag2, A2, B2, Dom, Pair).



%! marcxml_dom_field(+Dom, -Pair) is nondet.
%
% Control- or datafield.

marcxml_dom_field(element(datestamp,_,[Val1]), Key-Val2) :- !,
  strip_atom(Val1, Val2),
  rdf_equal(marc:datestamp, Key).
marcxml_dom_field(element('marc:leader',_,[Val1]), Key-Val2) :- !,
  strip_atom(Val1, Val2),
  rdf_equal(marc:leader, Key).
marcxml_dom_field(Dom, Pair) :-
  marcxml_dom_controlfield(Dom, Pair), !.
marcxml_dom_field(Dom, Pair) :-
  marcxml_dom_datafield(Dom, Pair).



%! marcxml_dom_record(+Dom, -Record) is det.

marcxml_dom_record([element('marc:record',_,Doms)], Record) :-
  findall(Pair, (member(Dom, Doms), marcxml_dom_field(Dom, Pair)), Pairs),
  sort(Pairs, Record),
  maplist(writeln, Record),
  nl.
  %dict_pairs(Record, marc, Pairs).
...
marcxml_dom_subfield(
  ...
  marc_subfield(Tag2, A2, B2, Code2, Val2, Pair).



%! marcxml_record(+Source, -Record) is nondet.

marcxml_record(Source, Record) :-
  xml_stream_record(
    Source,
    ['marc:record'],
    {Record}/[Dom]>>marcxml_dom_record(Dom, Record)
  ).
