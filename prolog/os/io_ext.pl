:- module(
  ...
  [
    ...
    write_stream_to_file/3  % +Read, +File, +Opts
  ]
).
...
@author Wouter Beek
@version 2015/08, 2015/10-2015/11, 2016/01-2016/02
*/
...
%! write_stream_to_file(+Read, +File, +Opts) is det.
% Options are passed to open_any2/5.

write_stream_to_file(Read, File) :-
  write_stream_to_file(Read, File, []).

write_stream_to_file(Read, File, Opts1) :-
  merge_options([type(binary)], Opts1, Opts2),
  setup_call_cleanup(
    open_any2(File, write, Write, Close_0, Opts2),
