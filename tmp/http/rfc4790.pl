...
% except that at first, the lower-case letters (octet values 97-122)
% in each input string are changed to upper case (octet values 65-90).
...
%   result "greater".
%
%   4. If both strings begin with the same octet value, remove the
%   first octet from both strings and repeat this algorithm from step
%   1.
%
%   5. If the unsigned value (0 to 255) of the first octet of the
%   first string is less than the unsigned value of the first octet of
%   the second string, then return "less".
%
...
% length equal to the length of the first string, which would result
% in a "match" result from the equality function.  Otherwise, the
